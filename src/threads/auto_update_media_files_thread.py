    ...
    def run(self) -> None:
        ...
        try:
            self.signals.progress.emit(
                10, "Beginning process of automatic update of media file(s)."
            )
            ...
            if self.directory_and_options["scan_extra_folder"]:
                self.signals.progress.emit(
                    25, "Prepping process to include extra folders in scan..."
                )
                generator_find_media_files = find_media_files_in_dir(
                    video_file_condition,
                    default_folder_condition,
                    self.directory_and_options["directory"],
                )
            else:
                self.signals.progress.emit(
                    25, "Skipping prepping process to include extra folders in scan..."
                )
                generator_find_media_files = find_media_files_in_dir(
                    video_file_condition,
                    skip_extra_folders,
            ...
            for file_list in generator_find_media_files:
                all_files_are_formatted_correctly = (
                    folder_and_file_pattern.check_files_in_list(file_list)
                )
                if not all_files_are_formatted_correctly:
                    # print("generate correct file formats") # debug
                    files_to_be_updated, message_number_of_files_affected = (
                        generate_correct_video_file_format(file_list)
                    )
            ...
            if (
                not message_number_of_files_affected
            ):  # For when no files were found that needed updating
                self.signals.progress.emit(
                    50, "-- No files were found that needed to be updated"
                )
